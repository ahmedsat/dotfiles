...
#-------------------------------------------------------------------------
# File: fancy-prompts.bash
#
# Inspired by Phil!'s ZSH Prompt: http://aperiodic.net/phil/prompt/
# Control your fancy Bash prompt from the command line.  Also available
# in a Zsh version.
#
# by BitJam, Version 1.0-rc01,  Wed Oct 31 13:59:19 MDT 2012
#-------------------------------------------------------------------------
...
#:
#:     source /some/path/fancy-prompts.bash
...
Config file: ~/.config/fancy-prompts-bash.conf
End_Usage
}
...
fancy-prompts-command() {
    ...
    if [ -n "$_PROMPT_TITLE_LEN" ]; then
        ...
        _PROMPT_TITLE_FILL="$(printf %${width}s)"
    fi

    _PROMPT_PWD=$PWD
    [ "$PWD" = "$HOME" -o -z "${PWD##$HOME/*}" ] && _PROMPT_PWD=\~${PWD#$HOME}
    local pwd_len=${#_PROMPT_PWD}

    unset _PROMPT_1_FILL _PROMPT_2_FILL
    # Used in -wide and -fancy
    if [ -n "$_PROMPT_1_LEN" ]; then
        if [[ "$pwd_len + $_PROMPT_1_LEN" -gt $term_width ]]; then
            # Truncate pwd to just fit
            local pwd_max=$(($term_width - $_PROMPT_1_LEN))
            local offset=$(($pwd_len - $pwd_max + 3))
            _PROMPT_PWD=...${_PROMPT_PWD:$offset}
        else
            # Fill bar to just fit
            _PROMPT_1_FILL=$(_prompt-fill $(($term_width - $_PROMPT_1_LEN - $pwd_len)))
        fi

    # Used in -zee
    elif [ -n "$_PROMPT_2_LEN" ]; then
...
_fancy-prompt() {
    ...
    [ -z "$BASH_VERSION" ] && _prompt-err "This does not look like a Bash shell" && return 2
    ...
    *) _prompt-err "_fancy_prompt() Unknown prompt type: $prompt_type" && return 3 ;;
    ...
    local top_lines="$(_prompt-fill "$line_cnt" "\\n")"
    ...
    # let colors default to "" if --nocolor
    local e=$(printf "\e[")
    ...
    local conf_file=$HOME/.config/fancy-prompts-bash.conf
    if [ -r "$conf_file" ]; then
    ...
    if [ -n "$title" ]; then
        _PROMPT_TITLE_LEN=${#title}
        title="\${_PROMPT_TITLE_FILL}$title_co$title$nc\n"
    fi
    ...
    local fill_1="\$_PROMPT_1_FILL"
    local fill_2="\$_PROMPT_2_FILL"
    local fill_3="\$_PROMPT_3_FILL"
    local p_pwd="\$_PROMPT_PWD"

    # Calculate these so there is no confusion over lengths
    local host=$(hostname)
    host=${host%%.*}
    local name=$(id -nu)
    ...
    local time_block="$line_co$lb$time_co\$(date +\"$time_fmt\")$line_co$rb"
    local date_block="$line_co$lb$date_co\$(date +\"$date_fmt\")$line_co$rb"
    ...
    # Get the length
    local time=$(date +"$time_fmt")
    local p2_bare="[$name@$host]$pad2[$time]"
    local p2_len=${#p2_bare}
    local date_bare=$(date +"$date_fmt")
    local date_len=${#date_bare}

    # NOTE: The trick used below is lazy processing by using \$ instead of $ in
    # some places.  If $ is used then the variable is evaluated now, if \$ is
    # used then the variable/code-block is evaluated when the prompt is drawn.
    # The _prompt_command() code is run right before the prompt is drawn and
    # this combination is what allows us to adjust the width of the prompts.
    ...
    dir)
        p3="$path_block$new_line"
        _PROMPT_1_LEN=$((2 + $margin))
        ;;

    med)
        p3="$p2$pad$path_block$new_line"
        _PROMPT_1_LEN=$(($p2_len + 3 + $margin))
        ;;

    narrow)
        p3="$p2$new_line$path_block$new_line"
        _PROMPT_1_LEN=$((2 + $margin))
        ;;

    gentoo)
        p3="$red\u $magenta\$(date +\"$time_fmt\") $green$p_pwd$new_line"
        local gentoo_bare="$name=$date="
        _PROMPT_1_LEN=$((${#gentoo_bare} + $margin))
        ;;

    wide)
        p3="$p2$pad$fill_1$path_block$new_line"
        _PROMPT_1_LEN=$(($p2_len + 2 + ${#pad} + $margin))
        ;;

    fancy)
        p3="$line_co$tl_corn$pad$p2$pad2$fill_1$path_block$nc$new_line"
        p3="$p3$line_co$bl_corn$pad"
        _PROMPT_1_LEN=$(($p2_len + ${#pad} + ${#pad2} + 3 + $margin))
        ;;

    zee)
        p3="$fill_2$path_block$pad$tr_corn$nc$new_line"
        p3="$p3$line_co$tl_corn$pad$p2$fill_3$pad$br_corn$nc$new_line"
        p3="$p3$line_co$bl_corn$pad"
        _PROMPT_2_LEN=$((3 + $margin + ${#pad}))
        _PROMPT_3_LEN=$(($p2_len + 2 * ${#pad} + 2 + $margin))
        ;;

    date)
        p3="$fill_2$path_block$pad$tr_corn$new_line"
        p3="$p3$line_co$tl_corn$pad$p2$fill_3$pad2$date_block$pad$br_corn$new_line"
        p3="$p3$line_co$bl_corn$pad"
        _PROMPT_2_LEN=$((3 + $margin + ${#pad}))
        _PROMPT_3_LEN=$(($p2_len + 2 * ${#pad} + ${#pad2} + 1 + $margin + $date_len + 2 + 1))
        ;;

    curl)
        p3="$line_co$tl_corn$pad$p2$fill_3$pad2$date_block$pad$tr_corn$new_line"
        p3="$p3$vbar$fill_2$path_block$pad$br_corn$new_line"
        p3="$p3$line_co$bl_corn$pad"
        _PROMPT_2_LEN=$((3 + $margin + ${#pad} + 1))
        _PROMPT_3_LEN=$(($p2_len + 2 * ${#pad} + ${#pad2} + 1 + $margin + $date_len + 2 + 1))
        ;;

    esac

    PS1="$top_lines$title$p3$prom_co$end_prompt$nc"
}
