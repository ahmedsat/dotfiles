...
var performanceAppraisalFile *os.File
var mu sync.Mutex

func init() {
	var err error
	performanceAppraisalFile, err = os.Create("PerformanceAppraisal.csv")
	if err != nil {
		panic(err)
	}
}
...
func GetAllFarms(tf Farm) (farms []Farm, err error) {
	...
	filters.Add("type", "=", "Farm")
	if tf.Name != "" {
		filters.Add("name", "=", tf.Name)
	}

	if tf.Region != "" {
		filters.Add("region", "=", tf.Region)
	}

	if tf.UpscalingProject != "" {
		filters.Add("upscaling_project", "=", tf.UpscalingProject)
	}
	...
	res, err := utils.RestGet(URL, filters, 0, 0, false)
	if err != nil {
		return
	}
...
func (f *Farm) ToCSV() (err error) {
	mu.Lock()
	defer mu.Unlock()

	_, err = io.Copy(performanceAppraisalFile, strings.NewReader(f.Name+","))
	if err != nil {
		return
	}

	_, err = io.Copy(performanceAppraisalFile, strings.NewReader(f.Region+","))
	if err != nil {
		return
	}

	_, err = io.Copy(performanceAppraisalFile, strings.NewReader("\n"))
	if err != nil {
		return
	}
